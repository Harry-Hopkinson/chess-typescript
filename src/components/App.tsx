...
const Chess: FC = () => {
	...
	const move = useCallback(
		(e: any) => {
			...
			const { pageX, pageY } = touch.changedTouches
				? touch.changedTouches[0]
				: mouse;

			if (!drag.current?.element) return e;

			drag.current.relativeY =
				(pageY - drag.current.startY) * (rotated ? -1 : 1);
			drag.current.relativeX =
				(pageX - drag.current.startX) * (rotated ? -1 : 1);
	...
	const end = useCallback(async () => {
		...
		const row =
			rows[newRowIndex < 0 ? 0 : newRowIndex > 8 ? 8 : newRowIndex];
		const column =
			columns[
				newColumnIndex < 0 ? 0 : newColumnIndex > 8 ? 8 : newColumnIndex
			];

		// Only submit valid moves to the reducer
		if (availableSquares.includes(`${column}${row}`)) {
			dispatch(
				actions.chessMove(`${piece.column}${piece.row}${column}${row}`)
			);
		}
		...
		() =>
			squares.map((square, index) => {
				...
				const start = (e: any) => {
					...
					if (draw || lost || ![turn, "both"].includes(userColor))
						return e;

					dispatch(
						actions.setAvailableSquares(
							`${square.piece.column}${square.piece.row}`
						)
					);

					const touch = e as TouchEvent;
					const mouse = e as MouseEvent;
					const { target } = touch.changedTouches
						? touch.changedTouches[0]
						: mouse;
					const element =
						boardElement.current?.querySelector(
							`.alive.square-${square.index} span`
						) || null;

					if (!element || !target || !boardElement.current) return;

					const edgeToCenter = Math.round(
						boardElement.current.clientWidth / 16
					);
				...
				return (
					<div
						...
						className={`board-square${
							latestMove.includes(coordinate)
								? " latest-move"
								: ""
							...
							availableSquares.includes(coordinate)
								? " available"
								: ""
						}`}
						key={index}
	...
	const pieceNodes = useMemo(
		() =>
			pieces.map((piece, index) => {
				...
				if (
					requestPromotion &&
					latestMove.indexOf(`${piece.column}${piece.row}`) === 0
				) {
				...
				return (
					<div
						...
						className={`piece ${piece.name}${
							piece.taken ? " taken" : " alive"
						} square-${
							rows.indexOf(piece.row) * 8 +
							columns.indexOf(piece.column)
						} ${
							dragging?.row === piece.row &&
							dragging?.column === piece.column
								? " dragging"
								: ""
						}`}
					>
	...
	return (
		<div className="Chess">
			...
			{!loading && (
				<div
					ref={boardElement}
					className={`board${rotated ? " rotated" : ""}${
						dragging ? " dragging" : ""
					}`}
				>
					{squareNodes}
					...
					<div
						className={`request-promotion ${
							requestPromotion ? ` show ${turn}` : ""
						}`}
					>
						<div
					...
					<div className="powered-by-stockfish">
						...
						<a
							...
							rel="noreferrer"
						>
							<i className="fab fa-github" /> Stockfish chess
							engine
						</a>
					</div>
				</div>
			...
			<div className="chess-controls">
				<Button
					...
					size="lg"
					className={
						!draw && !lost && userColor === "none" ? "active" : ""
					}
					onClick={() => {
						dispatch(
							actions.setChess({
				...
				>
					<i className="fas fa-laptop-code" /> vs{" "}
				...
				<Button
					...
					size="lg"
					className={
						!lost && !draw && userColor === "both" ? "active" : ""
					}
					onClick={() => {
						dispatch(
							actions.setChess({
